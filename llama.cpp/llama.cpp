...
// Be sure to compile with -Wmissing-field-initializers
...
void llama_print_tensor_info(struct ggml_tensor * tensor, const char* tensorName)
{
    ...
    //fprintf(stdout, "    n_layer = %d\n", hparams.n_layer);
    //fprintf(stdout, "    n_head = %d\n", hparams.n_head);
    //fprintf(stdout, "    n_embd = %d\n", hparams.n_embd);
    //fprintf(stdout, "    n_ctx = %d\n", hparams.n_ctx);
    //fprintf(stdout, "    n_vocab = %d\n", hparams.n_vocab);
}

void llama_print_model_info(struct llama_context * ctx)
{
    const auto & model = ctx->model;
    const auto & hparams = model.hparams;

    fprintf(stdout, "\n");
    fprintf(stdout, "Hyper parameter info:\n");
    fprintf(stdout, "    n_vocab = %d\n", hparams.n_vocab);
    fprintf(stdout, "    n_ctx = %d\n", hparams.n_ctx);
    fprintf(stdout, "    n_embd = %d\n", hparams.n_embd);
    fprintf(stdout, "    n_mult = %d\n", hparams.n_mult);
    fprintf(stdout, "    n_head = %d\n", hparams.n_head);
    fprintf(stdout, "    n_layer = %d\n", hparams.n_layer);
    fprintf(stdout, "    n_rot = %d\n", hparams.n_rot);
    fprintf(stdout, "    f16 = %d\n", hparams.f16);

    llama_print_tensor_info(model.tok_embeddings, "tok_embeddings");
    llama_print_tensor_info(model.layers[0].attention_norm, "0: attention_norm");
    llama_print_tensor_info(model.layers[0].wq, "0: wq");
    llama_print_tensor_info(model.layers[0].wk, "0: wk");
}

static int64_t llama_ggml_type_to_element_size(enum ggml_type type) {
    // Print ggml_type as string using a switch statement
    switch (type) {
