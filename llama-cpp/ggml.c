...
static bool gFirstCompute = true;
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        case GGML_OP_DUP:
            {
                if (gFirstCompute) { printf("OP: DUP %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_dup(params, tensor->src0, tensor);
            } break;
        case GGML_OP_ADD:
            {
                if (gFirstCompute) { printf("OP: ADD %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_add(params, tensor->src0, tensor->src1, tensor);
            } break;
        case GGML_OP_SUB:
            {
                if (gFirstCompute) { printf("OP: SUB %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_sub(params, tensor->src0, tensor->src1, tensor);
            } break;
        case GGML_OP_MUL:
            {
                if (gFirstCompute) { printf("OP: MUL %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_mul(params, tensor->src0, tensor->src1, tensor);
            } break;
        case GGML_OP_DIV:
            {
                if (gFirstCompute) { printf("OP: DIV %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_div(params, tensor->src0, tensor->src1, tensor);
            } break;
        case GGML_OP_SQR:
            {
                if (gFirstCompute) { printf("OP: SQR %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_sqr(params, tensor->src0, tensor);
            } break;
        case GGML_OP_SQRT:
            {
                if (gFirstCompute) { printf("OP: SQRT %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_sqrt(params, tensor->src0, tensor);
            } break;
        case GGML_OP_SUM:
            {
                if (gFirstCompute) { printf("OP: SUM %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_sum(params, tensor->src0, tensor);
            } break;
        case GGML_OP_MEAN:
            {
                if (gFirstCompute) { printf("OP: MEAN %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_mean(params, tensor->src0, tensor);
            } break;
        case GGML_OP_REPEAT:
            {
                if (gFirstCompute) { printf("OP: REPEAT %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_repeat(params, tensor->src0, tensor);
            } break;
        case GGML_OP_ABS:
            {
                if (gFirstCompute) { printf("OP: ABS %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_abs(params, tensor->src0, tensor);
            } break;
        case GGML_OP_SGN:
            {
                if (gFirstCompute) { printf("OP: SGN %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_sgn(params, tensor->src0, tensor);
            } break;
        case GGML_OP_NEG:
            {
                if (gFirstCompute) { printf("OP: NEG %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_neg(params, tensor->src0, tensor);
            } break;
        case GGML_OP_STEP:
            {
                if (gFirstCompute) { printf("OP: STEP %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_step(params, tensor->src0, tensor);
            } break;
        case GGML_OP_RELU:
            {
                if (gFirstCompute) { printf("OP: RELU %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_relu(params, tensor->src0, tensor);
            } break;
        case GGML_OP_GELU:
            {
                if (gFirstCompute) { printf("OP: GELU %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_gelu(params, tensor->src0, tensor);
            } break;
        case GGML_OP_SILU:
            {
                if (gFirstCompute) { printf("OP: SILU %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_silu(params, tensor->src0, tensor);
            } break;
        case GGML_OP_NORM:
            {
                if (gFirstCompute) { printf("OP: NORM %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_norm(params, tensor->src0, tensor);
            } break;
        case GGML_OP_RMS_NORM:
            {
                if (gFirstCompute) { printf("OP: RMS_NORM %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_rms_norm(params, tensor->src0, tensor);
            } break;
        case GGML_OP_MUL_MAT:
            {
                if (gFirstCompute) { printf("OP: MUL_MAT %d %d %d %ld %ld\n", params->type, params->ith, params->nth, tensor->src0, tensor->src1); }
                //int    n_dims;
        ...
        case GGML_OP_SCALE:
            {
                if (gFirstCompute) { printf("OP: SCALE %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_scale(params, tensor->src0, tensor->src1, tensor);
            } break;
        case GGML_OP_CPY:
            {
                if (gFirstCompute) { printf("OP: CPY %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_cpy(params, tensor->src0, tensor);
            } break;
        case GGML_OP_RESHAPE:
            {
                if (gFirstCompute) { printf("OP: RESHAPE %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_reshape(params, tensor->src0, tensor);
            } break;
        case GGML_OP_VIEW:
            {
                if (gFirstCompute) { printf("OP: VIEW %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_view(params, tensor->src0);
            } break;
        case GGML_OP_PERMUTE:
            {
                if (gFirstCompute) { printf("OP: PERMUTE %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_permute(params, tensor->src0);
            } break;
        case GGML_OP_TRANSPOSE:
            {
                if (gFirstCompute) { printf("OP: TRANSPOSE %d %d %d\n", params->type, params->ith, params->nth); }
        ...
        case GGML_OP_DIAG_MASK_INF:
            {
                if (gFirstCompute) { printf("OP: DIAG_MASK %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_diag_mask_inf(params, tensor->src0, tensor->src1, tensor);
            } break;
        case GGML_OP_SOFT_MAX:
            {
                if (gFirstCompute) { printf("OP: SOFT_MAX %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_soft_max(params, tensor->src0, tensor);
            } break;
        case GGML_OP_ROPE:
            {
                if (gFirstCompute) { printf("OP: ROPE %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_rope(params, tensor->src0, tensor->src1, tensor);
            } break;
        case GGML_OP_CONV_1D_1S:
            {
                if (gFirstCompute) { printf("OP: CONV_1D_1S %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_conv_1d_1s(params, tensor->src0, tensor->src1, tensor);
            } break;
        case GGML_OP_CONV_1D_2S:
            {
                if (gFirstCompute) { printf("OP: CONV_1D_2S %d %d %d\n", params->type, params->ith, params->nth); }
                ggml_compute_forward_conv_1d_2s(params, tensor->src0, tensor->src1, tensor);
            } break;
        case GGML_OP_FLASH_ATTN:
            {
                if (gFirstCompute) { printf("OP: FLASH_ATTN %d %d %d\n", params->type, params->ith, params->nth); }
        ...
        case GGML_OP_FLASH_FF:
            {
                if (gFirstCompute) { printf("OP: FLASH_FF %d %d %d\n", params->type, params->ith, params->nth); }
...
void ggml_graph_compute(struct ggml_context * ctx, struct ggml_cgraph * cgraph) {
    ...
    //std::shared_ptr<void> raiiFreeImage( 0, [&firstTime](void*) { firstTime = false; } );
